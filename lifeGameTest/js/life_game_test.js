describe('gameMap', function(){
    ...
    context('gameMap.change_next_state', function(){
    	it('should be a function', function(){
			assert.isFunction(gameMap.change_next_state);
		});
		it('should change life to next state rightly', function(){
			gameMap.row = 3;
			gameMap.col = 4;
			for(var i = 0; i < gameMap.row; i++){
				gameMap.grid[i] = [];
	        ...
	        assert.equal(gameMap.grid[0][0].life_state, 1);
            assert.equal(gameMap.grid[1][2].life_state, 0);
            assert.equal(gameMap.grid[1][1].life_state, 1);
		});   	
    });
